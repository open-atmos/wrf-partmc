...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  subroutine aero_dist_output_netcdf(aero_dists, n_times, ncid)

    !> Array of aerosol distributions.
    type(aero_dist_t) :: aero_dists(n_times)
    !>
    ...
    do i_mode = 1, n_modes
       radius(i_mode) = aero_dists(1)%mode(i_mode)%char_radius
    end do
    ...
    do i_mode = 1, n_modes
       std(i_mode) = aero_dists(1)%mode(i_mode)%log10_std_dev_radius
    end do
    ...
    do i_mode = 1, n_modes
       source(i_mode) = aero_dists(1)%mode(i_mode)%source
    end do
    ...
    do i_time = 1,n_times
    do i_mode = 1,n_modes
       num_conc(i_mode,i_time) = aero_dists(i_time)%mode(i_mode)%num_conc
    end do
    end do
    ...
    do i_time = 1,n_times
       do i_mode = 1,n_modes
          vol_frac(:,i_mode,i_time) = aero_dists(i_time)%mode(i_mode)%vol_frac
       end do
    end do
